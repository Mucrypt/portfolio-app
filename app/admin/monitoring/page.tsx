    ...
    <div className="p-8 max-w-full">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Real-time system metrics and activity logs</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(metrics?.health.status || "")}`}>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`w-3 h-3 rounded-full ${metrics?.cache.connected ? "bg-green-500" : "bg-red-500"}`}></span>
          </div>
          <div className="text-2xl font-bold text-gray-900 mb-1">
            {metrics?.cache.hitRate || "0%"}
          </div>
          <div className="text-xs text-gray-500">{metrics?.cache.keys || 0} cached keys • {metrics?.cache.memory || "0"}</div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="text-xs text-gray-500">{metrics?.performance.requestsPerMinute || 0} req/min</div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-2xl font-bold text-blue-900">{metrics?.cache.hits?.toLocaleString() || 0}</div>
          </div>
          <div className="p-4 bg-orange-50 rounded-lg">
            <div className="text-sm text-orange-600 mb-1">Cache Misses</div>
            <div className="text-2xl font-bold text-orange-900">{metrics?.cache.misses?.toLocaleString() || 0}</div>
          </div>
          <div className="p-4 bg-green-50 rounded-lg">
            <div className="text-sm text-green-600 mb-1">Hit Rate</div>
            <div className="text-2xl font-bold text-green-900">{metrics?.cache.hitRate || "0%"}</div>
          </div>
        </div>
      </div>
