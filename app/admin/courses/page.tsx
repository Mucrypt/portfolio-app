...
const platforms = ["Udemy", "Coursera", "Teachable", "LinkedIn Learning", "Pluralsight", "Skillshare", "Other"];
const categories = ["Web Development", "Mobile Development", "Backend", "DevOps", "Data Science", "Design", "Business", "Other"];
...
export default function AdminCoursesPage() {
  ...
  async function saveCourse() {
    ...
    const courseData = {
      ...
      students_count: formData.students_count ? parseInt(formData.students_count) : 0,
      what_you_learn: formData.what_you_learn ? formData.what_you_learn.split("\n").filter(Boolean) : [],
    ...
    <div>
      ...
      <div className="mb-8 p-6 border rounded-xl dark:bg-zinc-800">
        ...
        <div className="space-y-6">
          ...
          {/* Affiliate Link */}
          <div>
            <label className="block font-semibold mb-2">Affiliate Link * (Most Important!)</label>
            <input
              type="url"
              ...
              required
            />
            <p className="text-xs text-zinc-500 mt-1">Your affiliate link from the platform</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="block font-semibold mb-2">Rating (out of 5)</label>
              <input
                type="number"
                step="0.1"
                min="0"
          ...
          {/* What You'll Learn */}
          <div>
            <label className="block font-semibold mb-2">What You'll Learn (one per line)</label>
            <textarea
          ...
          {/* Requirements */}
          <div>
            <label className="block font-semibold mb-2">Requirements (one per line)</label>
            <textarea
      ...
      <div className="space-y-4">
        {courses.map((course) => (
          <div key={course.id} className="p-6 border rounded-xl dark:bg-zinc-800 flex justify-between items-start">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <h3 className="text-xl font-bold">{course.title}</h3>
                {course.is_featured && (
                  <span className="px-2 py-1 rounded bg-yellow-500 text-white text-xs font-bold">⭐ Featured</span>
                )}
                {!course.is_published && (
                  <span className="px-2 py-1 rounded bg-zinc-200 dark:bg-zinc-700 text-xs font-bold">Draft</span>
                )}
              </div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400 space-y-1">
                <p>👨‍🏫 {course.instructor_name} • 📚 {course.platform} • 🎯 {course.level}</p>
                <p>📁 {course.category} • ⏱️ {course.duration_hours}h • 💰 ${course.discounted_price || course.original_price}</p>
                <p className="text-xs text-purple-600 dark:text-purple-400 truncate">🔗 {course.affiliate_link}</p>
              </div>
            </div>
