...
function pickTopQuote(quotes: AboutQuoteRow[]) {
  const sorted = quotes.slice().sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
...
export default function AboutClient({
  ...
  const heroEyebrow = aboutPage?.hero_eyebrow ?? "🌍 Cameroon → Algeria → Italy → Software";
  const heroTitle =
  ...
  const originSection = sections.find((s) => (s.kind ?? "").toLowerCase() === "story");
  ...
  const timelineCards = useMemo(() => {
    ...
    const picked = prefer
      .map((c) => sorted.find((x) => x.country.toLowerCase() === c.toLowerCase()))
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-14 md:py-20">
        ...
        <section className="relative">
          <div className="js-hero inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-4 py-2 backdrop-blur">
            <span className="text-sm font-bold text-blue-700 dark:text-blue-300">{heroEyebrow}</span>
          </div>
            ...
            <div className="lg:col-span-4">
              <div className="js-hero-sub rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-950/40 backdrop-blur p-6 shadow-xl">
                ...
                <div className="mt-3 space-y-3 text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed">
                  <p>
                    <span className="font-bold text-zinc-900 dark:text-white">Craftsmanship</span>{" "}
                    taught me precision.
                  </p>
                  <p>
                    <span className="font-bold text-zinc-900 dark:text-white">Migration</span>{" "}
                    taught me resilience.
                  </p>
                  <p>
                    <span className="font-bold text-zinc-900 dark:text-white">Software</span>{" "}
                ...
                <div className="mt-5 pt-5 border-t border-zinc-200 dark:border-zinc-800 grid grid-cols-3 gap-3 text-center">
                  <div>
                    ...
                    <div className="text-xs text-zinc-500 dark:text-zinc-400">Countries</div>
                  </div>
                  <div>
                    <div className="text-xl font-black text-zinc-900 dark:text-white">2</div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400">Industries</div>
                  </div>
                  <div>
                    <div className="text-xl font-black text-zinc-900 dark:text-white">1</div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400">Mission</div>
                  </div>
                </div>
        ...
        <section ref={storyRef} className="mt-20 md:mt-28">
          <SectionTitle
            ...
            title={topQuote?.quote ?? "I didn’t come from privilege — I came from persistence."}
                ...
                <div className="relative">
                  ...
                  {x.title ? (
                    <p className="mt-2 text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                      {x.title}
                      {x.period ? <span className="text-zinc-500 dark:text-zinc-400"> • {x.period}</span> : null}
                    </p>
                  ) : null}
                  ...
                  {x.city ? (
                    <p className="mt-3 text-sm text-zinc-500 dark:text-zinc-400">📍 {x.city}</p>
                  ) : null}
                </div>
              </div>
            ))}
            ...
            <div className="relative">
              ...
              <h3 className="mt-4 text-3xl md:text-4xl font-black leading-tight">
                {topQuote?.quote?.includes("measure") ? (
                  <>
                    {topQuote.quote.split(":")[0] || "The same mindset builds a door and a product"}:
                    <span className="block bg-clip-text text-transparent bg-linear-to-r from-blue-300 via-purple-300 to-pink-300">
                      {topQuote.quote.includes("—") ? topQuote.quote.split("—").slice(1).join("—").trim() : "measure twice, ship once — and own the result."}
                    </span>
                  </>
                ) : (
                  <>
              ...
              <div className="mt-8 grid md:grid-cols-3 gap-4">
                {(craftHighlights.length
                  ? craftHighlights
                  : [
                      { id: "p1", title: "Precision", description: "clean UI, typed code, fewer bugs", icon: "🎯" },
                      { id: "p2", title: "Discipline", description: "consistent learning, consistent shipping", icon: "⚙️" },
                      { id: "p3", title: "Responsibility", description: "I don’t hide behind excuses — I deliver", icon: "🧱" },
                    ]
                ).map((x: any) => (
                  <div
                    key={x.id}
                  ...
                  >
                    ...
                    <div className="mt-3 font-black text-white">{x.title}</div>
                    <div className="mt-2 text-sm text-zinc-300 leading-relaxed">{x.description}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {(techCards.length
              ...
              >
                ...
                <div className="relative flex items-center gap-3">
                  ...
                  <div className="font-black text-zinc-900 dark:text-white">{tech.name}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="js-philo rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-950/40 backdrop-blur p-10 md:p-14 shadow-xl">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {(principleHighlights.length
                ? principleHighlights
                : [
                    { id: "a", title: "Build for real people", description: "If it doesn’t solve a real problem, it’s just decoration.", icon: "🧠" },
                    { id: "b", title: "Ship with responsibility", description: "I care about reliability, performance, and clean execution.", icon: "✅" },
                    { id: "c", title: "Learn fast, adapt faster", description: "New environment? New stack? I don’t panic — I learn.", icon: "🚀" },
                    { id: "d", title: "Respect the craft", description: "Whether it’s wood or code, quality is a mindset.", icon: "🪵" },
                  ]
              ).map((x: any) => (
                <div
                  key={x.id}
                ...
                >
                  <div className="text-2xl">{x.icon ?? "🧩"}</div>
                  <div className="mt-3 text-lg font-black text-zinc-900 dark:text-white">{x.title}</div>
                  <div className="mt-2 text-zinc-600 dark:text-zinc-300 leading-relaxed">{x.description}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section ref={closingRef} className="mt-20 md:mt-28 text-center">
          ...
          <p className="js-close mt-6 text-sm text-zinc-600 dark:text-zinc-400">
            If you want someone who can learn fast, ship clean, and bring discipline into products — let’s talk.
          </p>
        </section>
      </div>
    </div>
