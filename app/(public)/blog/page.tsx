...
import { Clock, Calendar, Eye, Heart, MessageCircle, Tag, TrendingUp, Bookmark } from "lucide-react";

export const metadata: Metadata = {
  title: "Blog | Romeo Mukula - Developer Insights & Tutorials",
  description: "Explore articles, tutorials, and insights on web development, mobile apps, DevOps, and modern software engineering practices.",
};
...
export default async function BlogPage({
  ...
  const pinnedPosts = posts.filter(post => post.is_pinned && !post.is_featured);
  const regularPosts = posts.filter(post => !post.is_featured && !post.is_pinned);
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 md:px-4 md:py-2 rounded-full mb-4 md:mb-6">
              ...
              <span className="text-xs md:text-sm font-medium">Developer Insights & Tutorials</span>
            </div>
            ...
            <p className="text-base sm:text-lg md:text-xl text-blue-100 mb-6 md:mb-8 max-w-2xl mx-auto px-4">
              Explore in-depth articles on web development, mobile apps, DevOps, and modern software engineering practices
            </p>
        ...
        <section className="container mx-auto px-4 -mt-8 md:-mt-16 relative z-20 mb-8 md:mb-16">
          <div className="bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl shadow-2xl p-4 md:p-8">
            <div className="flex items-center gap-2 mb-4 md:mb-6">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Featured Posts</h2>
            </div>
      ...
      <section className="container mx-auto px-4 py-6 md:py-12">
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-3 md:space-y-4">
              {pinnedPosts.length > 0 && (
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Latest Posts</h2>
              )}
            ...
            {posts.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set(posts.flatMap(post => post.tags || []))).slice(0, 15).map((tag) => (
                    <span
                      key={tag}
...
function BlogPostCard({ post, isPinned = false }: { post: BlogPost; isPinned?: boolean }) {
  return (
    <article className="bg-white dark:bg-gray-800 rounded-lg md:rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group">
      <div className="flex flex-col md:flex-row">
        {post.featured_image_url && (
          <Link href={`/blog/${post.slug}`} className="md:w-1/3 relative h-48 md:h-auto overflow-hidden">
            <Image
              src={post.featured_image_url}
              alt={post.title}
              fill
        ...
        <div className={`${post.featured_image_url ? "md:w-2/3" : "w-full"} p-4 md:p-6`}>
          {/* Author & Date */}
          <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm md:text-base text-gray-900 dark:text-white truncate">{post.author_name}</p>
              <div className="flex items-center gap-1.5 md:gap-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span className="hidden sm:inline">{post.reading_time_minutes} min</span>
              </div>
            </div>
            <span 
