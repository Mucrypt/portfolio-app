...
export function useClickTracking() {
  const trackClick = useCallback((element: HTMLElement, eventData?: Record<string, any>) => {
    const tagName = element.tagName.toLowerCase();
    const href = element.getAttribute('href');
    const text = element.textContent?.trim() || '';

    // Track external links
    if (tagName === 'a' && href && (href.startsWith('http') || href.startsWith('//'))) {
    ...
    if (href && socialPlatforms.some(platform => href.includes(platform))) {
      const platform = socialPlatforms.find(p => href.includes(p)) || 'unknown';
  ...
  useEffect(() => {
    ...
    const reportPerformance = () => {
      ...
      if (navigation) {
        ...
        gtag.trackTiming('page_load', Math.round(pageLoadTime), 'performance', 'full_page');

        // Track DNS lookup time
        const dnsTime = navigation.domainLookupEnd - navigation.domainLookupStart;
        gtag.trackTiming('dns_lookup', Math.round(dnsTime), 'performance', 'dns');

        // Track server response time
        const serverTime = navigation.responseEnd - navigation.requestStart;
        gtag.trackTiming('server_response', Math.round(serverTime), 'performance', 'server');

        // Track DOM content loaded time
        const domContentLoadedTime = navigation.domContentLoadedEventEnd - navigation.fetchStart;
        gtag.trackTiming('dom_content_loaded', Math.round(domContentLoadedTime), 'performance', 'dom');
      }

      // Track First Contentful Paint (FCP)
      const fcpEntry = performance.getEntriesByName('first-contentful-paint')[0];
      if (fcpEntry) {
        gtag.trackTiming('first_contentful_paint', Math.round(fcpEntry.startTime), 'performance', 'fcp');
      }
      ...
      const observer = new PerformanceObserver((list) => {
        ...
        gtag.trackTiming('largest_contentful_paint', Math.round(lastEntry.startTime), 'performance', 'lcp');
      });
